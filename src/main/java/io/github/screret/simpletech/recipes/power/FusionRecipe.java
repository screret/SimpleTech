...
public class FusionRecipe implements Recipe<Container> {
    ...
    public FusionRecipe(ResourceLocation resourceLocation, String group, int result, int usageTime, Ingredient ingredient) {
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRegistry.FUSION_RECIPE_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return ModRegistry.FUSION_RECIPE_TYPE;
    }

    public static class Serializer extends net.minecraftforge.registries.ForgeRegistryEntry<RecipeSerializer<?>> implements RecipeSerializer<FusionRecipe> {
        private static final ResourceLocation NAME = new ResourceLocation(SimpleTech.MOD_ID, PowergenMachineType.fusion.toString().toLowerCase(Locale.ROOT));
        public FusionRecipe fromJson(ResourceLocation recipe, JsonObject json) {
            ...
            return new FusionRecipe(recipe, s, result, usageTime, ingredient);
        }

        public FusionRecipe fromNetwork(ResourceLocation recipe, FriendlyByteBuf buffer) {
            String s = buffer.readUtf();
            int time = buffer.readVarInt();
            Ingredient ingredient = Ingredient.fromNetwork(buffer);

            int output = buffer.readInt();
            return new FusionRecipe(recipe, s, output, time, ingredient);
        }
